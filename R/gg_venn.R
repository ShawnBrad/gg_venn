...
#' 
#' @export
gg_venn<- function(overlap,  label.col = rep('black',15),label.size = 6, line.width = 2){
  ...
  if (length(overlap) == 2){
    setup.list(overlap = overlap) %>%
      venn_2way(overlap = overlap, label.col  = label.col , label.size = label.size,line.width = line.width ) %>%
      return()
